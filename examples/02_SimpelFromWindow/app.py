import sys

from PySide6.QtCore import Qt
from PySide6.QtGui import QPainter
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget,
                               QTableWidget, QHeaderView, QLineEdit,
                               QHBoxLayout, QPushButton, QFormLayout,
                               QVBoxLayout, QTableWidgetItem)
from PySide6.QtCharts import QChartView, QPieSeries, QChart


class FormWidget(QWidget):
    def __init__(self):
        super().__init__()
        self.items = 0

        # Example data
        self._data = {
            'Water': 24.5,
            'Electricity': 55.1,
            'Rent': 850.0,
            'Supermarket': 230.4,
            'Internet': 29.99,
        }

        # Left
        self.table = QTableWidget()
        self.table.setColumnCount(2)
        self.table.setHorizontalHeaderLabels(['Description', 'Price'])
        self.table.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)

        # Chart
        self.chart_view = QChartView()
        self.chart_view.setRenderHint(QPainter.Antialiasing)

        # Right
        self.description = QLineEdit()
        self.description.setClearButtonEnabled(True)
        self.price = QLineEdit()
        self.price.setClearButtonEnabled(True)

        self.add = QPushButton('Add')
        self.clear = QPushButton('Clear')
        self.plot = QPushButton('Plot')

        # Disabling 'Add' button
        self.add.setEnabled(False)
        form_layout = QFormLayout()
        form_layout.addRow('Description', self.description)
        form_layout.addRow('Price', self.price)
        self.right = QVBoxLayout()
        self.right.addLayout(form_layout)
        self.right.addWidget(self.add)
        self.right.addWidget(self.plot)
        self.right.addWidget(self.chart_view)
        self.right.addWidget(self.clear)

        # QWidget Layout
        self.layout = QHBoxLayout(self)
        self.layout.addWidget(self.table)
        self.layout.addLayout(self.right)

        # Signal and Slots
        self.description.textChanged.connect(self.check_disable_add)
        self.price.textChanged.connect(self.check_disable_add)
        self.clear.clicked.connect(self.clear_table)
        self.add.clicked.connect(self.add_element)
        self.plot.clicked.connect(self.plot_data)

        # Fill example data
        self.fill_table()

    def check_disable_add(self, s):
        enable = bool(self.description.text() and self.price.text())
        self.add.setEnabled(enable)

    def add_element(self):
        desc = self.description.text()
        price = float(self.price.text())

        self.table.insertRow(self.items)
        description_item = QTableWidgetItem(desc)
        price_item = QTableWidgetItem(f'{price:.2f}')
        price_item.setTextAlignment(Qt.AlignRight)
        self.table.setItem(self.items, 0, description_item)
        self.table.setItem(self.items, 1, price_item)

        self.description.clear()
        self.price.clear()

        self.items += 1

    def fill_table(self, data=None):
        data = self._data if not data else data
        for desc, price in data.items():
            description_item = QTableWidgetItem(desc)
            price_item = QTableWidgetItem(f'{price:.2f}')
            price_item.setTextAlignment(Qt.AlignRight)
            self.table.insertRow(self.items)
            self.table.setItem(self.items, 0, description_item)
            self.table.setItem(self.items, 1, price_item)
            self.items += 1

    def clear_table(self):
        print('clear table')
        self.table.setRowCount(0)
        self.items = 0

    def plot_data(self):
        series = QPieSeries()
        for i in range(self.table.rowCount()):
            desc = self.table.item(i, 0).text()
            price = float(self.table.item(i, 1).text())
            series.append(desc, price)
        chart = QChart()
        chart.addSeries(series)
        chart.legend().setAlignment(Qt.AlignmentFlag.AlignLeft)
        self.chart_view.setChart(chart)

class MainWindow(QMainWindow):
    def __init__(self, formWidget):
        super(MainWindow, self).__init__()
        self.setWindowTitle('Tutorial')

        # Menu
        menu = self.menuBar()
        file_menu = menu.addMenu("File")

        # Exit QAction
        file_menu.addAction("Exit", self.close)
        self.setCentralWidget(formWidget)


if __name__ == '__main__':
    # Qt Application
    app = QApplication([])
    # QWidget
    form = FormWidget()
    # QMainWindow using QWidget as central widget
    window = MainWindow(form)
    window.resize(800, 600)
    window.show()
    sys.exit((app.exec()))
